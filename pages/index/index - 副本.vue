    ...
    <view class="task-list">
      ...
      <view class="task-item" v-for="(task, idx) in tasks" :key="idx">
        ...
        <view class="task-row action-row">
          <view class="recruit recruit-normal">
            <text>招募</text>
            <text class="num">{{ task.needParticipants }}</text>
            <text>人</text>
          </view>
          <button class="join-btn" @click="joinTask(task)">立即加入</button>
        </view>
      </view>
    </view>
  </view>
...
export default {
  ...
  methods: {
    ...
    formatTasks(tasks) {
      ...
      return tasks.map(task => {
        ...
        return {
          ...
          needParticipants: task.needParticipants || 1
        };
      });
    },
    ...
    // 加入任务
    joinTask(task) {
      uni.reLaunch({ url: '/pages/publishDetails/publishDetails' });
    },
